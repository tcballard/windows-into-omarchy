...
```powershell
dotnet publish .\windows\WindowsIntoOnarchy\WindowsIntoOnarchy.csproj -c Release -r win-x64 --self-contained true -o .\dist\native-app
```

The project publishes a self-contained, single-file x64 WPF `WinExe` using the
original Windows Into Onarchy icon. Release packaging places the resulting
`WindowsIntoOnarchy.exe` at the application root beside `scripts`, `factory`,
