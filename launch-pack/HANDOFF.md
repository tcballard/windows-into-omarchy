...
- Pack status: verified artifact `9623937267` is pinned; permanent draft contains 10 matching assets; missing-asset upload pending
...
- Exact next action: merge the RC2 pinned-artifact recovery PR, run `prepare-v0.3.0-rc.2-bootstrap-release` on `main`, inspect the verified draft, then complete acceptance/review before choosing **Publish prerelease**
...
- Run `33041349157` proved that rebuilding the bundle is not byte-identical; the fail-closed resume check rejected it before upload. Recovery therefore uses only pinned artifact `9623937267` from run `33014004895`.
